...
A collection of powerful plugins that enhance the Verge3D puzzle editor with advanced functionality, keyboard shortcuts, and improved workflow tools. The suite includes **multi-selection**, **layout alignment**, a **minimap**, **cross-tab search**, **floating toolbar**, **procedure caller retarget**, and more.



Use the **Master** puzzle to chain every tool at once, or add individual puzzles  - some puzzles might require other puzzles to work.
...
**Usage:** Press **Shift+A** to search all available puzzles, variables, and procedures. Hold **Alt** when you want a **setter** instead of a **getter** for variables, or a **returning procedure** instead of a plain procedure when creating definitions from the menu.
...
**Usage:** Add **Enable Multi-Selection** to your master chain (after add menu, keyboard shortcuts, search, and minimap is recommended). Use Shift+click and box select to build a selection, then delete, duplicate, or open layout previews. See the block tooltip in the editor for the full shortcut list.
...
**Usage:** Add the **Procedure retarget (editor context menu)** puzzle to the **Master** chain (or place it once in the workspace). In Visual Logic, right-click a **caller** block → choose **Retarget all callers…** → enter the new procedure name.
...
**Usage:** In Visual Logic, add **Floating toolbar** after other Adam puzzles in the master chain, reload, and use the bar. Outside that editor context, the toolbar skips loading (see browser console).
